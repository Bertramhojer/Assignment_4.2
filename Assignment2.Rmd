...
a2_plot <- function(data, name){
  ggplot()+
    geom_line(aes(data$grid, data$pos), color = "red")+
    geom_line(aes(data$grid, data$pri), color = "blue")+
    geom_line(aes(data$grid, data$lik), color = "yellow")+
    labs(x = "probability grid",
         y = "Density distributions",
         title = paste(name, "distributions", sep = " "))
}
...
for (i in 1:4){
  ans <- paste("d", d19$Teacher[i], sep = "_")
  assign(ans, calc_teacher_knowledge(d19$Correct[i], d19$Questions[i], prior))
}

# Plotting posteriors for initial data
a2_plot(d_RF, "Riccardo")
a2_plot(d_MW, "Mikkel")
a2_plot(d_KT, "Kristian")
a2_plot(d_JS, "Josh")
```
